...
PendingClusterState::logConstructionInformation() const
{
    ...
    LOG(debug,
        ...
        "hash: '%s'",
        getPrevClusterState().toString().c_str(),
        getNewClusterState().toString().c_str(),
...
PendingClusterState::iAmDown() const
{
    const lib::NodeState& myState(
            getNewClusterState().getNodeState(Node(NodeType::DISTRIBUTOR,
...
PendingClusterState::printXml(vespalib::XmlOutputStream& xos) const
{
    ...
    xos << XmlTag("systemstate_pending")
        << XmlAttribute("state", getNewClusterState());
...
{
    return Summary(getPrevClusterState().toString(),
                   getNewClusterState().toString(),
